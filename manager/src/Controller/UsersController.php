...
class UsersController extends AbstractController
{
    ...
    public function edit(User $user, Request $request, Edit\Handler $handler): Response
    {
        if ($user->getId()->getValue() === $this->getUser()->getId()) {
            $this->addFlash('error', 'Нельзя редактировать свои данные.');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                $this->logger->warning($e->getMessage(), ['exception' => $e->getMessage()]);
                $this->addFlash('error', $e->getMessage());
            }
        }
    ...
    {
        if ($user->getId()->getValue() === $this->getUser()->getId()) {
            $this->addFlash('error', 'Нельзя изменить себе роль.');
            return $this->redirectToRoute('users.show', ['id' => $user->getId()]);
        }
